<script lang="ts">
  ...
  // Estado para controlar si los parámetros están desplegados o no
...
<div class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
  <!-- Cabecera con botón para expandir/colapsar -->
  <div class="flex justify-between items-center mb-4 cursor-pointer" on:click={() => isExpanded = !isExpanded}>
    <div class="flex items-center gap-2">
      <h3 class="text-lg font-medium">Parámetros del Shader</h3>
      <span class="text-xs text-gray-500 dark:text-gray-400">({$shaderParams.length} parámetros)</span>
      <svg 
          ...
          <div class="shader-param">
            ...
            {#if param.type === 'float' || param.type === 'int'}
              <div class="flex items-center space-x-2">
                ...
                />
                <span class="text-sm w-10 text-right">{(param.value).toFixed(2)}</span>
              </div>
...
{#if !isExpanded && $shaderParams.length > 0}
  <div class="mt-2 text-xs text-gray-500 dark:text-gray-400 text-center">
    Haz clic para mostrar/ocultar los {$shaderParams.length} parámetros
  </div>
{/if}
